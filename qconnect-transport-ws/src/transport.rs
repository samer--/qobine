...
///
/// The workspace compiles `rustls` with both the `ring` and `aws-lc-rs`
/// features (via `reqwest` 0.12 and `stream-download` → `reqwest` 0.13
/// respectively). `rustls` cannot auto-select a provider in that case, so we
/// install `ring` explicitly before the first TLS handshake.
